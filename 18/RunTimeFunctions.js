...
function drawStuff() {
  if (colorCount != 0)
  {
    for (x = 0; x < 50; x++)
    {
      for (y = 0; y < 50; y++)
      {
        field[x][y].flags = field[x][y].mines;
      }
    }
  }
    ...
    for(y = 0; y < sizeY; y++){
        ...
        for(i = 0; i < z.length; i++){
            ...
            if (field[sx][sy].mines != 0) {
              ...
              field[x][y].minesAround += field[sx][sy].mines; 
              field[x][y].mineSet.push(field[sx][sy].mines)
            }
            if((field[sx][sy].mines == -1)&&(field[x][y].minesAround <= 0)){
              field[x][y].minesAround += 33;
            }
            if(field[x][y].minesAround > 33){
              field[x][y].minesAround -= 33;
            }
        }
        field[x][y].mineSet.sort();
    }
  }
}
...
function setTimer(time, T){
  ...
  T.innerText = generations;
  ticks++;
  if ((ticks % 5 == 0 && autoGen == 1) || autoGen == 2) {
    entityCall('voidClickR', 0, 0)
    drawStuff();
    };
  }
...
function leftClick(posX, posY){
  ...
  if (field[posX][posY].mines == 1){
    if (colorCount == 0) {
      field[posX][posY].mines = 0;
      field[posX][posY].open = true;
    } else {
      field[posX][posY].mines = 2;
      field[posX][posY].flags = 2;
    }
  } else if (field[posX][posY].mines == 2) {
    if (colorCount == 2)
    {
      field[posX][posY].mines = 3;
      field[posX][posY].flags = 3;
    }
    else
    {
      field[posX][posY].mines = 0;
  ...
  } else if (field[posX][posY].mines == 3) {
    field[posX][posY].mines = -1;
    field[posX][posY].flags = -1;
  } else if (field[posX][posY].mines == -1) {
    field[posX][posY].mines = 0;
    field[posX][posY].flags = 0;
    field[posX][posY].open = true;
  } else {
    if (firstclicked == 0) {
      ...
      field[posX][posY].open = false;
      field[posX][posY].mines = 1;
      if (colorCount == 1) {field[posX][posY].flags = 1;}
...
function startNewGame(){
  ...
  Generator.neg = 0;
  if(colorCount == 0){
    Generator.amount = [1250,0,0];
  } else if (colorCount == 1) {
    Generator.amount = [625,625,0];
  } else {
    Generator.amount = [312,313,312]
    Generator.neg = 313;
  }
