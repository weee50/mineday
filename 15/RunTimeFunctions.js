...
function drawStuff() {
  ...
  var i;
  var j;
  cont.fillStyle = "#A19C98"
  cont.fillRect(0, 0, cont.canvas.width, cont.canvas.height)
  for(i = 0; i < sizeX; i++){
    for(j = 0; j < sizeY; j++){
      if(GameState == 3){
        if(field[i][j].mines != 0){
          cont.drawImage(Textures.FinalFlag[field[i][j].mines], screenscale * i, screenscale * j + (screenscale * i / 2), screenscale, screenscale);
          if(field[i][j].mines > 0){
          ...
          cont.drawImage(Textures.Ground[field[i][j].neighbours], screenscale * i, screenscale * j + (screenscale * i / 2), screenscale, screenscale);
          counterDisplay.Floor.c += 1;
        }
      }else if((field[i][j].mines != 0) && (GameState == 2)){
        if(field[i][j].open){
          cont.drawImage(Textures.OpenedMine[field[i][j].mines-1], screenscale * i, screenscale * j + (screenscale * i / 2), screenscale, screenscale);
        }else{
          cont.drawImage(Textures.Mine[field[i][j].mines-1], screenscale * i, screenscale * j + (screenscale * i / 2), screenscale, screenscale);
        }
      }else{
        if(field[i][j].open){
          cont.drawImage(Textures.Ground[field[i][j].neighbours], screenscale * i, screenscale * j + (screenscale * i / 2), screenscale, screenscale);
          ...
          cont.drawImage(Textures.Closed[field[i][j].flags], screenscale * i, screenscale * j + (screenscale * i / 2), screenscale, screenscale);
...
function startNewGame(){
  ...
  canvas.height = sizeY * screenscale + (sizeX * screenscale * 0.5 - (screenscale * 0.5));
  var i;
  var j;
  field = [];
  for(i = 0; i < sizeX; i++){
